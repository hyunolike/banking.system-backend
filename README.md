<div align="center">

<img width="1285" alt="image" src="https://user-images.githubusercontent.com/61215550/226171460-619789f1-9d8d-4653-8dc5-cb2a1adf7c64.png">

<br><br>

[![API Docs](http://img.shields.io/badge/-API%20Docs-important?style=flat&logo=dev.to&logoColor=white&link=https://hyunolike.github.io/banking.system-backend/#/)](https://hyunolike.github.io/banking.system-backend/#/)
[![Hits](https://hits.seeyoufarm.com/api/count/incr/badge.svg?url=https%3A%2F%2Fgithub.com%2Fhyunolike%2Fbanking.system-backend&count_bg=%2379C83D&title_bg=%23555555&icon=&icon_color=%23E7E7E7&title=hits&edge_flat=false)](https://hits.seeyoufarm.com)
</div>

<div align="center">
  <b>스프링 부트, 클라우드</b>를 이용해 요청부터 알림까지 <b>자동화 시스템</b>을 구축해보는 프로젝트 입니다 😁 <br>
</div>
<br> <br>

🚀 **프로젝트 개요**

- 아래와 같이 크게 3가지 서버로 구성되어 있습니다.
- **🖥️ Scheduler Server** 는 지정된 시간에 자동으로 요청을 보내는 서버입니다.
- **🖥️ API Server** 는 받은 요청에 따른 알림 서비스 제공을 위한 서버입니다.
- **🖥️ Notification Service**는 네이버 클라우드에서 제공하는 Application Services입니다.

<br>

📌 **프로젝트 목표**

- 짧은 기간내에 완성할 수 있도록 적절한 프레임워크 부가 기능을 사용한다.
- 요구사항에 적절한 기술을 선택해 쉽게 개발을 진행한다.
- Github Actions을 이용해 배포 자동화를 구축한다.

<br>

🧑🏻‍💻 **팀원 소개**
|[🌱 장현호](https://github.com/hyunolike)|
|:---:|
|<a href="https://github.com/hyunolike"> <img src="https://avatars.githubusercontent.com/hyunolike" width=200px alt="장현호"/> </a>|
|백엔드|

<br>

## 주요 서버 소개
### 🖥️ Scheduler Server
#### 🏰 기술 스택 
<img width="1405" alt="image" src="https://user-images.githubusercontent.com/61215550/226174295-4a5f1afa-ed2d-4ef6-b3cf-9f9c33663f62.png">

### 🖥️ API Server
#### 🏰 기술 스택 
<img width="1405" alt="image" src="https://user-images.githubusercontent.com/61215550/226174263-b27e83bf-2b91-4363-b088-ed3d2615fe48.png">

## 프로젝트 구조
(🚧 공사중 ... )



<a rel="license" href="http://creativecommons.org/licenses/by-nc-nd/4.0/"><img alt="크리에이티브 커먼즈 라이선스" style="border-width:0" src="https://i.creativecommons.org/l/by-nc-nd/4.0/88x31.png" /></a><br /><span xmlns:cc="http://creativecommons.org/ns#" property="cc:attributionName">장현호</span>에 의해 작성된 <span xmlns:dct="http://purl.org/dc/terms/" property="dct:title">automated banking system</span>은(는) <a rel="license" href="http://creativecommons.org/licenses/by-nc-nd/4.0/">크리에이티브 커먼즈 저작자표시-비영리-변경금지 4.0 국제 라이선스</a>에 따라 이용할 수 있습니다.
